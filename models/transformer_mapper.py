...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, y: Optional[T] = None, mask: Optional[T] = None):
...
class TransformerLayer(nn.Module):
    ...
    def forward_with_attention(self, x, y: Optional[T] = None, mask: Optional[T] = None):
...
def main():
    net = TransformerEncoderDecoder(512, 768, 10)
    x = torch.rand(3, 512)
    out = net(x)
    print(out.shape)
